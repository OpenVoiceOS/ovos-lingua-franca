...
class TestExtractTimeSpan(unittest.TestCase):
    def test_extract_timespan_replace_token(self):
        self.assertEqual(extract_timespan("10 seconds", replace_token="_"),
                         (timedelta(seconds=10.0), "_ _"))
        self.assertEqual(extract_timespan("The movie is 1 hour, 57.5 minutes long",
                                          replace_token="_"),
                         (timedelta(hours=1, minutes=57.5),
                          "The movie is _ _, _ _ long"))
        # TODO remainder is imperfect
        #  "fifty seven and a half" is normalized to a single word "57.5"
        self.assertEqual(extract_timespan("The movie is one hour, fifty seven"
                                          " and a half minutes long", replace_token="_"),
                         (timedelta(hours=1, minutes=57.5),
                          "The movie is _ _, _ _ long"))
    ...
    def test_extract_timespan_en(self):
        self.assertEqual(extract_timespan("10 seconds"),
                         (timedelta(seconds=10.0), ""))
        self.assertEqual(extract_timespan("5 minutes"),
                         (timedelta(minutes=5), ""))
        self.assertEqual(extract_timespan("2 hours"),
                         (timedelta(hours=2), ""))
        self.assertEqual(extract_timespan("3 days"),
                         (timedelta(days=3), ""))
        self.assertEqual(extract_timespan("25 weeks"),
                         (timedelta(weeks=25), ""))
        self.assertEqual(extract_timespan("seven hours"),
                         (timedelta(hours=7), ""))
        self.assertEqual(extract_timespan("7.5 seconds"),
                         (timedelta(seconds=7.5), ""))
        self.assertEqual(extract_timespan("eight and a half days thirty"
                                          " nine seconds"),
                         (timedelta(days=8.5, seconds=39), ""))
        self.assertEqual(extract_timespan("Set a timer for 30 minutes"),
                         (timedelta(minutes=30), "Set a timer for"))
        self.assertEqual(extract_timespan("Four and a half minutes until"
                                          " sunset"),
                         (timedelta(minutes=4.5), "until sunset"))
        self.assertEqual(extract_timespan("Nineteen minutes past the hour"),
                         (timedelta(minutes=19), "past the hour"))
        self.assertEqual(extract_timespan("wake me up in three weeks, four"
                                          " hundred ninety seven days, and"
                                          " three hundred 91.6 seconds"),
                         (timedelta(weeks=3, days=497, seconds=391.6),
                          "wake me up in , , and"))
        ...
        self.assertEqual(extract_timespan("1 month"),
                         (timedelta(days=DAYS_IN_1_MONTH), ""))
        self.assertEqual(
        ...
        self.assertEqual(extract_timespan("3 months"),
                         (timedelta(days=DAYS_IN_1_MONTH * 3), ""))
        self.assertEqual(extract_timespan("a year"),
                         (timedelta(days=DAYS_IN_1_YEAR), ""))
        self.assertEqual(extract_timespan("1 year"),
                         (timedelta(days=DAYS_IN_1_YEAR * 1), ""))
        self.assertEqual(extract_timespan("5 years"),
                         (timedelta(days=DAYS_IN_1_YEAR * 5), ""))
        self.assertEqual(extract_timespan("a decade"),
                         (timedelta(days=DAYS_IN_1_YEAR * 10), ""))
        self.assertEqual(extract_timespan("1 decade"),
                         (timedelta(days=DAYS_IN_1_YEAR * 10), ""))
        self.assertEqual(extract_timespan("5 decades"),
                         (timedelta(days=DAYS_IN_1_YEAR * 10 * 5), ""))
        self.assertEqual(extract_timespan("1 century"),
                         (timedelta(days=DAYS_IN_1_YEAR * 100), ""))
        self.assertEqual(extract_timespan("a century"),
                         (timedelta(days=DAYS_IN_1_YEAR * 100), ""))
        self.assertEqual(extract_timespan("5 centuries"),
                         (timedelta(days=DAYS_IN_1_YEAR * 100 * 5), ""))
        self.assertEqual(extract_timespan("1 millennium"),
                         (timedelta(days=DAYS_IN_1_YEAR * 1000), ""))
        self.assertEqual(extract_timespan("5 millenniums"),
                         (timedelta(days=DAYS_IN_1_YEAR * 1000 * 5), ""))
    ...
    def test_extract_timespan_milliseconds_en(self):
        def test_milliseconds(duration_str, expected_duration,
                              expected_remainder):
            duration, remainder = extract_timespan(
    ...
    def test_extract_timespan_seconds_en(self):
    ...
    def test_extract_timespan_minutes_en(self):
    ...
    def test_extract_timespan_hours_en(self):
    ...
    def test_extract_timespan_days_en(self):
    ...
    def test_extract_timespan_weeks_en(self):
